...
const { exportArtboards, getExportArtboardDatas, getExportArtboards } = require('../utils/export');
...
class App extends React.Component {
  ...
  onExportCurrent() {
    ...
    this.setState({ disabled: true }, () => {
      exportArtboards(this.state.appID, selectedArtboard, this.state.device, teamID, info => {
  ...
  onExportAll() {
    ...
    const { teamID } = this.state;
    this.setState({ disabled: true }, () => {
      exportArtboards(this.state.appID, artboards, this.state.device, teamID, info => {
        const roleInTeam = this.getRoleInTeam();
        if (roleInTeam === 'guest') {
          this.props.showMessage(i18n('error.noPermission'));
          return;
  ...
  checkState() {
    ...
    getExportArtboardDatas(selectedArtboard, this.state.device).then(res => {
      ...
      getAllCopy(appID, clientID).then(res => {
        if (res.length) {
          getExportArtboards(this.state.appID, selectedArtboard, this.state.device, teamID)
            .then(res => {
              this.setState({ newStateItem: res[0] });
            })
        ...
        loadApps(teamID)
          .then(apps => {
            ...
            if (apps && apps.length) {
              ...
              const curApp = allApp.filter(item => !item.isAppSet)[0];
              let appID = curApp && curApp._id;
              this.setState({ appID });
            }
    ...
    return (
      <div className="c-app">
        ...
        <div className="actions" disabled={disabled}>
          ...
          >
            <span>{selApp ? selApp.name : i18n('app.chooseProject')}</span>
        ...
        <div>
          ...
          <button className="update-all app-btn" disabled={!canExportAll} onClick={this.onExportAll}>
            {i18n('app.uploadAll')}
          </button>
          <div className="state-box app-btn">
            <button className="update-state" disabled={!canExportAll} onClick={this.checkState}>
              {i18n('multiStatus.uploadstateImg')}
            </button>
            <img
              src="/images/icons/about.png"
              className={`${isMac.isMac ? 'about' : 'is-mac-about'}`}
              onClick={() => {
                this.setState({ stateTip: true });
              }}
            />
          </div>
        </div>
        <div className="app-bottom">
          <Version />
          <a
            className={`my-app${selApp ? '' : ' disabled-link'}`}
        ...
        />
        {this.state.showTip && (
          <UploadTip url={`${urlCfg.homePage}app/${appID}/storyboard`} num={num} onClose={this.onCloseTip} />
        )}
