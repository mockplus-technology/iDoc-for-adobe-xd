...
class CreateAppPanel extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      appName: scenegraph.selection.focusedArtboard?scenegraph.selection.focusedArtboard.name:'',
    };
  ...
  render() {
   ...
   let appSetListData = appSetList.map(item=>{return {name:item.name,id:item._id}})
    appSetListData.unshift({name:i18n('app.noSelectAppSet'),id:''})
    const selectAppSet = appSetList.find(item => item._id === selApp.appSetID)
    if (selectAppSet && appSetID === '') {
      this.setState({
    ...
    return (
      <div className="create-panel" onClick={this.props.onClose}>  
        <div className="content" onClick={(e)=>{e.stopPropagation()}}>      
          ...
          <input className="app-name" placeholder={i18n('app.projectName')} value={this.state.appName} onChange={this.onInputChange} ref={(el)=>{  this.inputTitle = el;}}/>
          <div className="select-appSet" onClick={(e) => {
            e.stopPropagation();
          ...
          <RadioGroup
            ...
            items={[{ text: i18n('app.phoneProject'), value: 'phone' }, { text: i18n('app.webProject'), value: 'web' }]}
            onChange={(value) => {
              this.setState({ appType: value })
            }}
          />
          <button onClick={this.onSubmit} className="app-btn">{i18n('app.create')}</button>
          <button onClick={this.props.onClose} className="app-btn">{i18n('app.cancel')}</button>
          <div className="appSet-list">
          {showAppSetPanel && <ListBox
            maxHeight={240}
            Selected={appSetID}
            onSelected={(item)=>{this.setState({appSetName: item.name, appSetID:item.id, showAppSetPanel:false })}}
